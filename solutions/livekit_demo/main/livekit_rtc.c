...
int livekit_user_packet_handler(livekit_user_packet_t *user_packet, livekit_room_state_t *room)
{
...
int livekit_rpc_request_handler(livekit_rpc_request_t *rpc_request, livekit_room_state_t *room)
{
...
int livekit_rpc_ack_handler(livekit_rpc_ack_t *rpc_ack, livekit_room_state_t *room)
{
...
int livekit_rpc_response_handler(livekit_rpc_response_t *rpc_response, livekit_room_state_t *room)
{
...
int livekit_stream_header_handler(livekit_data_stream_header_t *stream_header, livekit_room_state_t *room)
{
...
int livekit_stream_chunk_handler(livekit_data_stream_chunk_t *stream_chunk, livekit_room_state_t *room)
{
...
int livekit_stream_trailer_handler(livekit_data_stream_trailer_t *stream_trailer, livekit_room_state_t *room)
{
...
int livekit_rtc_data_channel_handler(uint8_t *data, int size, livekit_room_state_t *room)
{
    ...
    switch (data_packet.which_value) {
        case LIVEKIT_DATA_PACKET_USER_TAG:
            return livekit_user_packet_handler(&data_packet.value.user, room);
        case LIVEKIT_DATA_PACKET_RPC_REQUEST_TAG:
            return livekit_rpc_request_handler(&data_packet.value.rpc_request, room);
        case LIVEKIT_DATA_PACKET_RPC_ACK_TAG:
            return livekit_rpc_ack_handler(&data_packet.value.rpc_ack, room);
        case LIVEKIT_DATA_PACKET_RPC_RESPONSE_TAG:
            return livekit_rpc_response_handler(&data_packet.value.rpc_response, room);
        case LIVEKIT_DATA_PACKET_STREAM_HEADER_TAG:
            return livekit_stream_header_handler(&data_packet.value.stream_header, room);
        case LIVEKIT_DATA_PACKET_STREAM_CHUNK_TAG:
            return livekit_stream_chunk_handler(&data_packet.value.stream_chunk, room);
        case LIVEKIT_DATA_PACKET_STREAM_TRAILER_TAG:
            return livekit_stream_trailer_handler(&data_packet.value.stream_trailer, room);
        default:
...
int livekit_rtc_data_handler(esp_webrtc_custom_data_via_t via, uint8_t *data, int size, void *ctx)
{
    ...
    switch (via) {
        case ESP_WEBRTC_CUSTOM_DATA_VIA_DATA_CHANNEL:
            return livekit_rtc_data_channel_handler(data, size, room);
        case ESP_WEBRTC_CUSTOM_DATA_VIA_SIGNALING:
            return livekit_rtc_signaling_data_handler(data, size, room);
        default: break;
    }
    return 0;
}

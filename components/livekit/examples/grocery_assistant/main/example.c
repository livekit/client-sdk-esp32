...
/// Invoked by a remote participant to get the current scale weight reading.
static void read_scale_kg(const livekit_rpc_invocation_t* invocation, void* ctx)
{
    float weight;
    if (!scale_read(&weight)) {
        livekit_rpc_return_error("Failed to read scale");
        return;
    }
    weight /= 1000.0f; // Convert to kg
    char weight_string[32];
    snprintf(weight_string, sizeof(weight_string), "%.4f", weight);
    livekit_rpc_return_ok(weight_string);
}
...
void join_room()
{
    ...
    livekit_room_rpc_register(room_handle, "read_scale_kg", read_scale_kg);
